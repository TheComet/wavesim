...
public:
    virtual void SetUp()
    {
        ASSERT_THAT(hash_set_create(&hs, HM_REFERENCE_KEYS), Eq(WS_OK));
    }
...
TEST_F(NAME, hash_collisions)
{
    ...
    for (int i = 0; i != 8; ++i)
        EXPECT_THAT(hash_set_add_str(hs, buf[i]), Ne(HASH_SET_ERROR));
    EXPECT_THAT(hash_set_count(hs), Eq(8u));

    for (int i = 0; i != 8; ++i)
        EXPECT_THAT(hash_set_find_str(hs, buf[i]), Ne(HASH_SET_ERROR));
    EXPECT_THAT(hash_set_find_str(hs, "nonexistent"), Eq(HASH_SET_ERROR));
}
