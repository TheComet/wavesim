 ...
 * ------------------------------------------------------------------------- */
vector_t*
vector_create(const uintptr_t element_size)
{
...
/* ------------------------------------------------------------------------- */
void
vector_construct(vector_t* vector, const uintptr_t element_size)
{
    assert(vector);
...
/* ------------------------------------------------------------------------- */
uintptr_t
vector_resize(vector_t* vector, uintptr_t size)
{
    assert(vector);
...
/* ------------------------------------------------------------------------- */
void*
vector_emplace_multi(vector_t* vector, uintptr_t size)
{
    void* data;
...
/* ------------------------------------------------------------------------- */
void
vector_swap(vector_t* vector, uintptr_t index1, uintptr_t index2)
{
...
/* ------------------------------------------------------------------------- */
void*
vector_insert_emplace(vector_t* vector, uintptr_t index)
{
    assert(vector);
    ...
    else
    {
        ...
        uintptr_t offset = vector->element_size * index;
        uintptr_t total_size = vector->count * vector->element_size;
        void* src = (void*)((uintptr_t)vector->data + offset);
        void* dst = (void*)((uintptr_t)vector->data + offset + vector->element_size);
        uintptr_t bytes = total_size - offset;
...
/* ------------------------------------------------------------------------- */
int
vector_insert(vector_t* vector, uintptr_t index, void* data)
{
    void* emplaced;
...
/* ------------------------------------------------------------------------- */
void
vector_erase_index(vector_t* vector, uintptr_t index)
{
    assert(vector);
    ...
    else
    {
        ...
        uintptr_t offset = vector->element_size * index;  /* offset to the element being erased in bytes */
        uintptr_t total_size = vector->element_size * vector->count; /* total current size in bytes */
...
/* ------------------------------------------------------------------------- */
void*
vector_get(const vector_t* vector, uintptr_t index)
{
    assert(vector);
    assert(index < vector->count);
              ...
              uintptr_t target_count)
{
    ...
    else
    {
        ...
        uintptr_t offset = vector->element_size * insertion_index;
        uintptr_t total_size = vector->element_size * vector->count;
